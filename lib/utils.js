var deepEqual = module.exports.deepEqual = function (pattern, chemical) {
  if(Array.isArray(pattern)) {
    if(pattern.length != chemical.length)
      return false
    for(var i = 0; i<pattern.length; i++)
      if(!deepEqual(pattern[i], chemical[i]))
        return false
    return true
  }
  else
  if(typeof pattern == "object") {
    for(var key in pattern)
      if(!deepEqual(pattern[key], chemical[key]))
        return false
    return true
  } else
  if(typeof pattern == "function" && chemical instanceof pattern) {
    return true
  } else
  return pattern === chemical
}

module.exports.isFilledArray = function (arr) {
  for(var i = 0; i<arr.length; i++)
    if(arr[i] == undefined || typeof arr[i] == "undefined")
      return false
  return true
}


module.exports.isChemicalInSet = function (chemical, chemicalSet) {
  for(var i = 0; i < chemicalSet.length; i++) {
    var chemicalFromSet = chemicalSet[i]
    if(deepEqual(chemical, chemicalFromSet)) {
      return true
    }
  }
  return false
}
